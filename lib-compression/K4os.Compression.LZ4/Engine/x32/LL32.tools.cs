...
namespace K4os.Compression.LZ4.Engine.x32
{
	...
	#endif
	{
		...
		private static readonly uint* DeBruijnBytePos = Mem32.CloneArray(_DeBruijnBytePos);
		...
		protected static uint LZ4_count(byte* pIn, byte* pMatch, byte* pInLimit)
		{
			...
			{
				var diff = Mem32.PeekW(pMatch) ^ Mem32.PeekW(pIn);
				if (diff != 0)
			...
			if (pIn < pInLimit - 1 && Mem32.Peek2(pMatch) == Mem32.Peek2(pIn))
			{
				pIn += 2;
				pMatch += 2;
			}
		...
		protected static uint LZ4_hashPosition(void* p, tableType_t tableType)
		{
			...
			#endif
			return LZ4_hash4(Mem32.Peek4(p), tableType);
		}
