	...
	#endif
	{
			...
			int acceleration)
		{
			...
			for (;;)
			{
				...
				if (tableType == tableType_t.byPtr)
				{
					...
					while ((match + LZ4_DISTANCE_MAX < ip) || (Mem32.Peek4(match) != Mem32.Peek4(ip)));
				}
				else
				{
					/* byU32, byU16 */
					...
					do
					{
						...
						if (Mem32.Peek4(match) == Mem32.Peek4(ip))
						{
...

				{
					...
					Mem32.WildCopy8(op, anchor, op + litLength);
					op += litLength;
				}
				...
				if (maybe_extMem)
				{
					...
					Mem32.Poke2(op, (ushort) offset);
					op += 2;
				}
				else
				{
					Assert(ip - match <= LZ4_DISTANCE_MAX);
					Mem32.Poke2(op, (ushort) (ip - match));
					op += 2;
				}
				...
				if (tableType == tableType_t.byPtr)
				{
					...
					LZ4_putPosition(ip, cctx->hashTable, tableType, @base);
					if ((match + LZ4_DISTANCE_MAX >= ip) && (Mem32.Peek4(match) == Mem32.Peek4(ip)))
					{
						token = op++;
						*token = 0;
						goto _next_match;
				...
				else
				{
					...
					if (((dictIssue != dictIssue_directive.dictSmall)
						...
						&& (((tableType == tableType_t.byU16)
								&& (LZ4_DISTANCE_MAX == LZ4_DISTANCE_ABSOLUTE_MAX))
							|| (matchIndex + LZ4_DISTANCE_MAX >= current))
						&& (Mem32.Peek4(match) == Mem32.Peek4(ip)))
					{
						token = op++;
						*token = 0;
						if (maybe_extMem) offset = current - matchIndex;
			...
			_last_literals:
			{
				...
				Mem32.Copy(op, anchor, (int) lastRun);
				ip = anchor + lastRun;
				op += lastRun;
			}
			...
			int acceleration)
		{
...

			{
				...
				{
					if (inputSize > 4 * KB) {
						Mem32.Copy((byte*)streamPtr, (byte*)streamPtr->dictCtx, sizeof(LZ4_stream_t));
