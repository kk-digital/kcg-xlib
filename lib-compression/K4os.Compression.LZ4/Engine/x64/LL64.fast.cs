	...
	#endif
	{
			...
			int acceleration)
		{
			...
			for (;;)
			{
				...
				if (tableType == tableType_t.byPtr)
				{
					...
					while ((match + LZ4_DISTANCE_MAX < ip) || (Mem64.Peek4(match) != Mem64.Peek4(ip)));
				}
				else
				{
					/* byU32, byU16 */
					...
					do
					{
						...
						if (Mem64.Peek4(match) == Mem64.Peek4(ip))
						{
...

				{
					...
					Mem64.WildCopy8(op, anchor, op + litLength);
					op += litLength;
				}
				...
				if (tableType == tableType_t.byPtr)
				{
					...
					LZ4_putPosition(ip, cctx->hashTable, tableType, @base);
					if ((match + LZ4_DISTANCE_MAX >= ip) && (Mem64.Peek4(match) == Mem64.Peek4(ip)))
					{
						token = op++;
						*token = 0;
						goto _next_match;
				...
				else
				{
					...
					if (((dictIssue != dictIssue_directive.dictSmall)
						...
						&& (((tableType == tableType_t.byU16)
								&& (LZ4_DISTANCE_MAX == LZ4_DISTANCE_ABSOLUTE_MAX))
							|| (matchIndex + LZ4_DISTANCE_MAX >= current))
						&& (Mem64.Peek4(match) == Mem64.Peek4(ip)))
					{
						token = op++;
						*token = 0;
						if (maybe_extMem) offset = current - matchIndex;
			...
			_last_literals:
			{
				...
				Mem64.Copy(op, anchor, (int) lastRun);
				ip = anchor + lastRun;
				op += lastRun;
			}
			...
			int acceleration)
		{
...

			{
				...
				{
					if (inputSize > 4 * KB) {
						Mem64.Copy((byte*)streamPtr, (byte*)streamPtr->dictCtx, sizeof(LZ4_stream_t));
