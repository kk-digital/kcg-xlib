...
namespace K4os.Compression.LZ4.Engine.x64
{
	#if BIT32
...

			{
				...
				while (true)
				{
					...
					*/
					...
					{
						/* Copy the literals */
						if (endOnInput) Mem64.Copy16(op, ip);
						else Mem64.Copy8(op, ip);
						...
						|| ((!endOnInput) && (cpy > oend - WILDCOPYLENGTH)))
					{
						...
						*/
						...
						Mem64.Move(op, ip, (int) length); /* supports overlapping memory regions, which only matters for in-place decompression scenarios */
						ip += length;
						op += length;
					...
					if ((dict == dict_directive.usingExtDict) && (match < lowPrefix))
					{
						...
						if (length <= (size_t) (lowPrefix - match))
						{
							...
							Mem64.Move(op, dictEnd - (lowPrefix - match), (int) length);
							op += length;
						}
						else
						{
							...
							Mem64.Copy(op, dictEnd - copySize, (int) copySize);
							op += copySize;
							...
							{
								Mem64.Copy(op, lowPrefix, (int) restSize);
								op += restSize;
							}
						}
					...
					if ((offset < 8))
					{
						...
						Mem64.Copy4(op + 4, match);
						match -= dec64table[offset];
					}
					else
					{
						Mem64.Copy8(op, match);
						match += 8;
					}
					...
					if ((cpy > oend - MATCH_SAFEGUARD_DISTANCE))
					{
						...
						if (op < oCopyLimit)
						{
							Mem64.WildCopy8(op, match, oCopyLimit);
							match += oCopyLimit - op;
							op = oCopyLimit;
						}
					...
					{
						Mem64.Copy8(op, match);
						if (length > 16) { Mem64.WildCopy8(op + 8, match + 8, cpy); }
					}
