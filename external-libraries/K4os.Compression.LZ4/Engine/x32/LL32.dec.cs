...
using Mem = K4os.Compression.LZ4.Internal.x32.Mem32;
#else
using reg_t = System.UInt64;
using Mem = K4os.Compression.LZ4.Internal.Mem64;
#endif
using K4os.Compression.LZ4.Internal.x32;
using size_t = System.UInt32;
using uptr_t = System.UInt64;

//------------------------------------------------------------------------------

namespace K4os.Compression.LZ4.Engine.x32
{
	#if BIT32
...

			{
				...
				while (true)
				{
					...
					*/
					...
					{
						/* Copy the literals */
						if (endOnInput) Mem32.Copy16(op, ip);
						else Mem32.Copy8(op, ip);
						...
						|| ((!endOnInput) && (cpy > oend - WILDCOPYLENGTH)))
					{
						...
						*/
						...
						Mem32.Move(op, ip, (int) length); /* supports overlapping memory regions, which only matters for in-place decompression scenarios */
						ip += length;
						op += length;
					...
					if ((dict == dict_directive.usingExtDict) && (match < lowPrefix))
					{
						...
						if (length <= (size_t) (lowPrefix - match))
						{
							...
							Mem32.Move(op, dictEnd - (lowPrefix - match), (int) length);
							op += length;
						}
						else
						{
							...
							Mem32.Copy(op, dictEnd - copySize, (int) copySize);
							op += copySize;
							...
							{
								Mem32.Copy(op, lowPrefix, (int) restSize);
								op += restSize;
							}
						}
					...
					if ((offset < 8))
					{
						...
						Mem32.Copy4(op + 4, match);
						match -= dec64table[offset];
					}
					else
					{
						Mem32.Copy8(op, match);
						match += 8;
					}
					...
					if ((cpy > oend - MATCH_SAFEGUARD_DISTANCE))
					{
						...
						if (op < oCopyLimit)
						{
							Mem32.WildCopy8(op, match, oCopyLimit);
							match += oCopyLimit - op;
							op = oCopyLimit;
						}
					...
					{
						Mem32.Copy8(op, match);
						if (length > 16) { Mem32.WildCopy8(op + 8, match + 8, cpy); }
					}
