...
namespace K4os.Compression.LZ4.Engine.x64
{
	...
	#endif
	{
		...
		public static uint LZ4HC_countPattern(byte* ip, byte* iEnd, uint pattern32)
		{
			...
			{
				reg_t diff = Mem64.PeekW(ip) ^ pattern;
				if (diff == 0)
				{
					ip += ARCH;
					continue;
				...
				HCfavor_e favorDecSpeed)
		{
			...
			while ((matchIndex >= lowestMatchIndex) && (nbAttempts != 0))
			{
				...
				else if (matchIndex >= dictLimit)
				{
					...
					Assert(matchPtr < ip);
					Assert(longest >= 1);
					if (Mem64.Peek2(iLowLimit + longest - 1)
						== Mem64.Peek2(matchPtr - lookBackLength + longest - 1))
					{
						if (Mem64.Peek4(matchPtr) == pattern)
						{
			...
			byte* oend)
		{
			...
			/* Copy Literals */
			Mem64.WildCopy8(*op, *anchor, (*op) + length);
			*op += length;
			...
			dictCtx_directive dict)
		{
			...
			*srcSizePtr = 0;
			...
			/* Encode Last Literals */
			{
				...
				Mem64.Copy(op, anchor, (int) lastRunSize);
				op += lastRunSize;
			}
...
/* ~64 KB, which is a bit large for stack... */
			...
			limitedOutput_directive limit)
		{
			...
			{
				Mem64.Copy((byte*) ctx, (byte*) ctx->dictCtx, sizeof(LZ4_streamHC_t));
